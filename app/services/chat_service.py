...
class ChatService:
    ...
    def get_or_create_session(self,session_id: Optional[str] = None) -> str:
        ...
        if not session_id:
            ...
            logger.info("[TIMING] session_get_or_create: %.3fs (news)",time.perf_counter() - t0)
            return new_session_id
        
        if not self.validate_session_id(session_id):
            raise ValueError(
                f"Invalid session_id format: {session_id}. Session ID must be non-empty,"
                "not contain path traversal characters, and be under 255 character."
            )
    ...
    def process_message(self, session_id: str, user_message: str) -> str:
        ...
        logger.info("[GENERAL] Response length: %d chars | Preview %1.120s", len(response), response)
        return response
    ...
    def process_realtime_message(self, session_id: str, user_message: str) -> str:
        ...
        self.add_message(session_id,"assistant", response)
        logger.info("[REALTIME] Response length: %d chars | Preview %1.120s", len(response), response)
        return response
