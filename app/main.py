...
async def lifespan(app: FastAPI):
    ...
    logger.info("[CONFIG] Groq API keys loaded: %s", len(GROQ_API_KEYS))
    ...
    try:
        ...
        logger.info("[TIMING] startup_vector_service: %.3fs", time.perf_counter() - t0)

        logger.info("Initializing Groq service (general queries)...")
        groq_service = GroqService(vector_store_service)
        logger.info("Groq service initialized successfully")

        logger.info("Initializing Realtime service (with Tavily search)...")
...
class TimingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        logger.info("[REQUEST] %s %s → %s (%.3fs)", request.method, request.url.path, response.status_code, elapsed)
        return response
...
# ───────────────────────────────────────────────
#       Streaming helpers (TTS + sentence splitting)
# ───────────────────────────────────────────────
...
def _split_sentences(buf: str):
    """Split text into sentences, keeping short fragments for next chunk"""
...
async def chat_realtime(request: ChatRequest):  # ← fixed type hint (was ChatResponse)
    if not chat_service:
        raise HTTPException(status_code=503, detail="Chat service not initialized")
